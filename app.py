...
def generate_content_based_recommendations(profile, user_registrations):
    ...
    try:
        # Perform filtered vector search - increased top_k to get more results
        results = search_similar_programs(full_query, filters=filters, top_k=50)
        ...
        for match in results:
            ...
            if is_program_available_in_month(
        ...
        # Return top 10 content-based recommendations
...
def merge_recommendations(content_based, collaborative, user_registrations):
    ...
    # Sort by score (content-based first, then collaborative)
...
def get_recommendations():
    ...
    # Get collaborative recommendations - increased limit
    ...
    return jsonify({"recommendations": merged_recommendations})
...
def recommend_programs():
    ...
    try:
        # Increased top_k to get more search results
        results = search_similar_programs(full_query, filters=filters, top_k=50)
        recommendations = []
